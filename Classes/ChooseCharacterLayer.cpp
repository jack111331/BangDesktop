...
bool ChooseCharacterLayer::onTouchBegan(Touch *touch, Event *event) {
    ...
    log("[ChooseCharacterLayer] locationInNode=(%f, %f)", locationInNode.x, locationInNode.y);
    ...
    if (rect.containsPoint(locationInNode)) {
        // if we want to use end, we must let began detect and return true
        log("[ChooseCharacterLayer] touched layer began");
        return true;
    }
    return false;
}
...
void ChooseCharacterLayer::onTouchEnded(Touch *touch, Event *event) {
    log("[ChooseCharacterLayer] touched layer ended");
